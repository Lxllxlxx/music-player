<template>
    <div class="play">
        <!-- 模糊背景(靠样式设置), 固定定位 -->
        <div class="song-bg" :style="`background-image: url(${songInfo && songInfo.al && songInfo.al.picUrl}`"></div>
        <!-- 播放页头部导航 -->
        <div class="header">
            <van-icon name="arrow-left" size="20" class="left-incon" @click="re()" />
        </div>
        <!-- 留声机 - 容器 -->
        <div class="song-wrapper">
            ...
            <div class="song-turn ani" :style="`animation-play-state:${playState ? 'running' : 'paused'}`">
                <div class="song-img">
                    ...
                    <img style="width: 100%" :src="`${songInfo && songInfo.al && songInfo.al.picUrl}`" alt="" />
                </div>
            </div>
            <!-- 播放按钮 -->
            ...
            <div class="song-msg">
                ...
                <div class="lrcContent">
                    <p class="lrc" style="height: 5px;">{{ curLyric }}</p>
                </div>
            </div>
            <!-- 留声机 - 唱臂 -->
            <div class="needle" :style="`transform: rotate(${needleDeg});`"></div>
            <div style="text-align: center;margin-top: 150px;">
                <van-icon name="arrow-left" size="3rem" color="lightblue" style="margin-right: 50px;"
                    @click="up()"></van-icon>
                <van-icon name="like-o" size="3rem" color="#ee0a24" v-if="liked == false" @click="zan()"></van-icon>
                <van-icon name="like" size="3rem" color="#ee0a24" v-if="liked == true" @click="remove()"></van-icon>
                <van-icon name="arrow" size="3rem" color="lightblue" style="margin-left: 50px;" @click="down()"></van-icon>
            </div>
        </div>
...
export default {
    ...
    computed: {
        ...
        filMusic() {
            if (this.user != null) {
                return this.likeMusic.filter((p) => {
                    // console.log(p.phone);
    ...
    mounted() {
        this.getSong()
        this.showLyric()
        this.likeMusic = JSON.parse(localStorage.getItem("userLike"))
    ...
    methods: {
        ...
        zan() {
            if (this.user == null) {  //用户未登录 进行提示
                showFailToast('请先登录！')
            } else {
                this.liked = true
                this.likeMusic.push({  //将歌曲信息存入喜欢列表中
                    id: this.id, name: this.songInfo.name, artist: this.songInfo.ar[0].name, album: this.songInfo.al.name,
                    pic: this.songInfo.al.picUrl, liked: true, phone: this.user.phone
                })
                localStorage.setItem("userLike", JSON.stringify(this.likeMusic))
            }
        },
        remove() {
            this.filMusic.map(item => {
            ...
            //在收藏列表中找到符合条件的记录进行删除
            let index = this.likeMusic.findIndex((arr) => arr.id == this.id && arr.phone == this.user.phone);
            // console.log(index);
        ...
        down() {
            //到达播放列表末尾时
            let index = this.playlist.findIndex((arr) => arr.id == this.id) + 1
            if (index == this.playlist.length) {
                index = 0
            }
            this.id = this.playlist[index].id
            this.$router.replace('/play' + '?songid=' + this.id)
        },
        up() {
             //到达播放列表开头时
            let index = this.playlist.findIndex((arr) => arr.id == this.id) - 1
